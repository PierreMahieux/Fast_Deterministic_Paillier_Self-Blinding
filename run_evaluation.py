...
DEFAULT_LENGTH_SIGNATURE = 512
DEFAULT_MESSAGE_LENGTH = 521
DEFAULT_QUANTISATION_FACTOR = 4
...
if __name__ == "__main__":
    ...
    if "message_length" not in config:
        original_message_length = DEFAULT_MESSAGE_LENGTH
    else:
        original_message_length = config["message_length"]
    if "self_blinding_length" not in config:
        original_self_blinding_length = DEFAULT_LENGTH_SIGNATURE
    else:
        ...
        if "list_qim_step" in config:
            for delta in config["list_qim_step"]:
                ...
                for method in list_methods:
                    if "message_length" not in config:
                        config["message_length"] = DEFAULT_MESSAGE_LENGTH
                    ...
                    if "self_blinding_length" not in config:
                        config["self_blinding_length"] = DEFAULT_LENGTH_SIGNATURE
                    ...
                    config.update({"result_folder": result_folder, "model_path": model_path, "model_name": model_name, "signature_length": DEFAULT_LENGTH_SIGNATURE, "method_name": method.__name__})
