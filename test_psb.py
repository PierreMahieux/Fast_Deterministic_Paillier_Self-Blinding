...
from src.probabilist_self_blinding import psb, embedding, extracting, preprocessing
...
if __name__ == "__main__":
    ...
    result_folder = os.path.join(script_dir, f"./results/psb/{model_name.split(".")[0]}/")
    result = {}
    ...
    encrypted_vertices, result_preprocess = psb.preprocessing(model["vertices"], encryption_keys, config)
    result = result | result_preprocess

    signed_vertices, result_embedding = psb.embedding(encrypted_vertices, watermark_bits, {"encryption": encryption_keys, "signing": signing_keys}, config)
    result = result | result_embedding

    decrypted_vertices, result_extracting = psb.extracting(signed_vertices, {"encryption": encryption_keys, "signing": signing_keys}, config["quantisation_factor"], config["message_length"], config["signature_length"], config["qim_step"])
    result = result | result_extracting

    recovered_mesh = psb.recover_mesh(decrypted_vertices, encryption_keys["public"], config["quantisation_factor"])
